...
- **Production Tracker — sticky phase columns + JCS-based cost variance:**
  - Sticky columns: cost code (165px), description (270px), unit (50px) stay pinned when horizontally scrolling. CSS-only via `.pt-sticky-1/2/3` + `.pt-sticky-group` classes; `.prod-tracker-tbl` switched to `border-collapse:separate` so sticky positioning works. Right-edge shadow on the rightmost sticky column marks the pinned/scrolling boundary.
  - JCS-based variance — replaces useless budget−actual with **JCS planned-to-date − actual**. New helper `computeJCSPlannedByCode(job, refDate)` walks every activity envelope and pro-rates each `directTotal` between `_barStart` and `_barEnd` at today, bucketed by `phaseCode`. Production Tracker now shows 4 cost columns: **Budget $** (total), **JCS Plan $** (what should have been spent by now), **Actual $** (with confidence pill + tooltip), **JCS Var** (Plan − Actual; positive = under plan, negative = over).
  - KPI tile **Cost Variance** renamed/repurposed to **JCS Variance** with sublabel "$X planned · $Y actual" so the user sees both halves at a glance.
  - Group header label updated to "Cost (vs Job Cost Schedule)" — colspan bumped from 3 to 4. Drill-down row colspan bumped from 17 to 18.

### 2026-05-04 — Session 12
- **Cost blend rewritten — per-BELMOS-bucket MAX(Spectrum JTD, HJ lifetime).** Replaces the old "Spectrum ≥30d + HJ <30d sum" approach. Garrett's reasoning: the higher number is correct because (a) Spectrum has burden added to labor/equip when it posts, (b) field guys often don't track sub costs in HJ at all (Spectrum picks them up via AP), (c) materials get under-logged or skipped in HJ but show up in Spectrum invoicing, (d) the SUM rule was double-counting when Spectrum posted recent items faster than expected. New rule captures all four cases naturally — whichever source has more dollars wins per bucket.
- **`blendedActualsByCode` rewrite (public/index.html ~line 2935):** dropped the `r.date < cutoffISO` filter on both HJ paths (`actualsDetail` + `hcssJobCosts`) so HJ side now represents lifetime per code. Combine logic switched from `b.spectrum.X + b.hj.X` to `Math.max(b.spectrum.X, b.hj.X)` per BELMOS bucket. Total = sum of per-bucket maxes (not max of totals — that would lose codes where Spectrum is higher on labor but HJ is higher on mat). Hours and qty also use max() per the same rule.
- **Confidence tags reduced to 4 (was 6):**
  - `cleaned` — Spectrum > HJ; trust Spectrum (burden / AP / sub posted)
  - `live` — HJ > Spectrum; real costs accruing, Spectrum hasn't caught up. Likely accurate, just unreconciled — judgment required.
  - `investigate` — values differ by >50% AND gap >= $500. Real anomaly worth a human look.
  - `none` — both 0.
  Worst-of-row grade: investigate < live < cleaned. Old tags (`clean`, `reconciled`, `mixed`, `flagged`) retired. `BLEND_INVESTIGATE_GAP=0.5` and `BLEND_INVESTIGATE_MIN_$=500` are tunable constants.
- **Source map labels:** simplified to `spectrum-reconciled` (Spectrum side won), `hj-canonical` (HJ side won), or `none`.
- **`formatBlendTooltip` rewritten** to show "Spectrum $X (JTD) · HJ $Y (lifetime) → using $Z (greater per bucket)" instead of "Spectrum $X (through cutoff) + HJ $Y (since cutoff) = $Z". The cutoff date is still passed through for callers that want it but isn't rendered in the string.
- **`confidencePill` palette:** rebuilt for the 4-tag scheme. Cleaned uses the old reconciled blue, live keeps its orange, investigate uses the old flagged red.
- **`confidenceTooltipFor`** rewritten to show per-component max() decision: each row lists Labor/Equip/Material/Subcontract with the tag and both Spectrum + HJ values so the user can see why one won.
- **JCS view (`renderJobCost`) brought into line** with the new rule. Dropped the 30-day filter on `hjByCode` (HJ rows now lifetime), per-row blendedActual = `max(jtd, hjTotal)`, phase-group blendedActual = `max(g.jtd, g.hjLifetime)`, total blended actual = sum of per-row maxes. Header tooltip on the "Actual (blended)" column updated. The legacy `hj30` field renamed to `hjLifetime`.
- **Production Tracker confidence aggregate banner** updated to show only the 3 active tags (cleaned / live / investigate). Stale `BLEND_CUTOFF_DAYS = 30` constant kept (no longer slices dollars; kept as historical reference for any caller that still wants to know the 30-day window).
- **Pending deploy:** front-end only. `deploy.command` to push `public/index.html`. After Netlify rebuilds, refresh and verify: (1) Production Tracker pills show CLEANED/LIVE/INVESTIGATE on rows that have data; (2) JCS "Actual (blended)" KPI matches Production Tracker's totActualCost for the same job; (3) hover the Actual $ column to see the new tooltip "Spectrum $X (JTD) · HJ $Y (lifetime) → using $Z (greater per bucket)".
- **Memory updated:** `feedback_blend_cutoff.md` rewritten — the cutoff is no longer load-bearing for dollars, only for tooltip context. New rule is per-bucket MAX.
- **Confidence-tagged source-aware merge (Pass 1 of #26):**
  - `blendedActualsByCode` extended to consume both HJ data paths (`actualsDetail` from xlsx + timecard sync, AND `hcssJobCosts` from `/jobCosts/advancedRequest` API). Greater-of within HJ side per BELMOS component to avoid double-counting overlapping rows.
  - Per-component confidence tags emitted on every blend bucket: `clean` (HJ Labor/Equip — gospel), `reconciled` (Spectrum-only mat/sub, posted), `live` (HJ-30d-only mat/sub, uncleaned, judgment required), `mixed` (both Spectrum and HJ-30d contributed), `flagged` (data quality issue), `none`. Worst-of-row grade = `confidenceGrade` exposed on `blend[code].confidenceGrade`.
  - Source map per component: `hj-gospel`, `spectrum-jtd`, `spectrum+hj-30d`, `hj-30d-uncleaned`, `none`.
  - `applyHcssJobCostsToActuals` REWRITTEN to use `blendedActualsByCode` instead of greater-of merge. Each `job.actuals` row now carries `_confidence`, `_confidenceGrade`, `_sourceMap`, `_spectrumComponents`, `_hjComponents`, `_blendCutoff`. Old greater-of logic preserved as `_legacyApplyHcssJobCostsToActuals` for fallback.
  - **UI surface:** Production Tracker rows display a confidence pill next to the cost code (CLEAN/RECONCILED/MIXED/LIVE/FLAGGED) with hover tooltip showing per-component breakdown. New aggregate banner above the table shows total $ split by confidence grade with code counts. Tooltip per pill explains the rule.
  - Helpers: `confidencePill(grade, tooltipHtml)`, `confidenceTooltipFor(actualsRow)`.
  - Self-test extended: `blendedActualsByCode emits confidence tags` per job; `confidencePill` + `confidenceTooltipFor` + `blendedActualsByCode` added to required-fn list.
- **Verification toolkit — Layer 1 (in-app self-test) + Layer 2 (pre-deploy gate) shipped:**
  - **Layer 1:** `runSelfTest()` exposed on window. Battery of ~10 invariants — every job has a clean `actuals` array, all required helper fns exist, divergence/stuck/unit-cost compute without throwing, tie-out check that `Σ bidItems.directTotal ≈ Σ activities.directTotal` (within 1%), pace-mode toggle persists, notification settings round-trip. Renders results as a sticky top banner with a Details panel listing every check + message. Stores last result in `localStorage.bdc_selftest_last`. Trigger via `?selftest=1` URL param OR "Run Self-Test" button in Settings → Notifications. `runSelfTestAndShow()` wraps run + banner.
  - **Layer 2:** `deploy.command` now runs a Node-based syntax check on `public/index.html` before pushing — extracts every inline `<script>` block, parses each via `new Function()`, aborts the push if any block throws a SyntaxError. Output identifies which block + line. Bypass with `SKIP_SYNTAX_CHECK=1` if needed. Skipped silently if Node isn't installed.
  - **Layer 3 (scheduled validation):** queued for setup via Cowork's schedule skill after deploy lands — daily 6am run hitting the live site with `?selftest=1`, alerts on failure.
- **BUG FIX (Pass 2) — sanitizer added to `loadJobs`:** even after fixing the writers, the corrupted `job.actuals` shape (sparse array with null holes from numeric-string indexing) is still in Supabase. Added an idempotent sanitizer in `loadJobs` after `migrateTaskMap`: filters non-object / null rows, drops rows without a `code`, dedupes by `code` keeping the highest-actualCost row, ensures the result is a plain array. Runs on every job on every load. After this deploys + the user refreshes, `saveJob` will rewrite the clean version back to Supabase so the persisted data heals on the next save.
- **BUG FIX — array vs object confusion in `job.actuals` corrupted Steel Driver:** Symptom: switching the active job from Stone Blvd to Steel Driver left the JCS / Daily Insights showing Stone Blvd numbers. Root cause: `job.actuals` is built by `rollupActualsDetail` as `Object.values(byCode)` — an ARRAY. My `applyHcssJobCostsToActuals` from the Pass 2 work treated it as a keyed object (`if(!job.actuals)job.actuals={}; job.actuals[code]={}`), corrupting the structure. `renderJobCostSchedule` then threw `Cannot read properties of null (reading 'code')` on the corrupted array, the render aborted silently, and the previous job's view stayed on screen. Patched: `applyHcssJobCostsToActuals` now treats `job.actuals` as an array and uses a Map index for upserts. Also fixed `computeStuckCodes`, `computeBidVsActualUnitCost`, `computeJobScheduleDivergence`, `computeScopeAdjustmentByPhase` — all originally iterated `for(const code in job.actuals)` which gives array INDEXES, not cost codes, when the value is an array. All now normalize via `Array.isArray(job.actuals) ? job.actuals : Object.values(job.actuals||{})` and read `a.code` from each row. **Pending deploy.**

### 2026-08-14 — Session (Nic, via Claude Code in DirtOS session)
- **Fixed hcss-sync-metadata-nightly (2am UTC cron) — was silently 401ing every night for 10+ days.** Nic reported new HeavyJob projects (jobs 792 "Tronox road repair", 793 "Prairie Property") weren't showing up anywhere. pg_cron's `cron.job_run_details` showed "succeeded" every run — misleading, since `net.http_post` is async and that only means the request was queued, not that it got a 200. The real response (`net._http_response`) showed `{"ok":false,"error":"Unauthorized"}` on every call: this job builds its bearer token from `vault.decrypted_secrets`, which is stale/wrong. The sibling `hcss-sync-actuals-daily` cron (jobid 18) uses a literal hardcoded token and is confirmed working right now — fixed `hcss-sync-metadata-nightly` in place to match (see migration `20260814220000_fix_hcss_metadata_sync_secret.sql`). Also widened `net.http_post`'s timeout from the 5s default to 5 min — a full metadata sync pulls jobs + per-job cost codes across ~140 jobs with HCSS's required 250ms inter-request pause, which can exceed a short timeout.
- **Verified live, not just "should work":** manually re-fired the corrected call once and watched `hcss_jobs` go from 141 → 143 rows in real time, confirming the two new jobs above are now synced. No front-end or Edge Function code changed — this was a DB-only fix (pg_cron job definition), applied directly via Supabase MCP, not through `deploy.command` or `supabase functions deploy`.
- Also (briefly) added a second, since-removed duplicate cron before finding the pre-existing one — no lasting effect, cleaned up in the same session.
- **Not yet done:** update the front-end's Setup tab messaging/HANDOFF docs to reflect that metadata sync is now automatic nightly, not manual-only. Garrett may still want the manual "Sync Jobs & Cost Codes" button for on-demand refreshes — left untouched.

### 2026-08-18 — Session (Nic, via Claude Code in DirtOS session)
- **Probed the sync's own `scanEndpoints` diagnostic mode live** (read-only, wrote nothing) to answer "are we pulling everything HeavyJob's API allows." Found two real gaps and closed both — same session as the DirtOS Data Quality tab work (see that repo's decisions.md for the "actuals_detail_sync cost columns are dead" finding that prompted this).
- **Fixed `hcss-sync-job-costs-batch-0/1/2` — same stale-vault-secret 401 bug as the metadata cron fixed 2026-08-14, this time on the job-costs sync.** `hcss_job_costs` (real $ per code/date/foreman from `jobCosts/advancedRequest`) hadn't updated since 2026-08-11 — 7 days silently stale. Root cause identical: `vault.decrypted_secrets['hcss_sync_secret']` doesn't match the real `HCSS_SYNC_TOKEN`. **Also found:** all three "batch" crons passed `{batchIndex, batchCount}` in their body, but `syncJobCosts` in `index.ts` has zero code reading either field — every "batch" was silently running the FULL active-job list, three times a day, for no benefit. Consolidated to one `hcss-sync-job-costs-daily` cron with the corrected literal token + a 300s timeout (see `20260818170000_fix_hcss_job_costs_cron.sql`). Verified live: manual fire returned 200, `hcss_job_costs` picked up 177 new rows / 2 new jobs, current through the prior day — the sync's default 35-day lookback recovered the missed week on its own, no explicit backfill needed.
- **Surfaced HCSS's per-cost-code BUDGET dollars, which were already being fetched and silently discarded into a JSONB blob.** The `costCodes?jobId=` response (pulled by the metadata sync) includes `laborDollars/equipmentDollars/materialDollars/subcontractDollars/supplyDollars/laborHours/equipmentHours` per cost code — already stored verbatim in `hcss_cost_codes.raw`, just never extracted. Added 7 `GENERATED ALWAYS ... STORED` columns (`budget_labor_dollars` etc., see `20260818170100_hcss_cost_code_budget_columns.sql`) computed directly from `raw`, so they can't drift out of sync and need zero Edge Function change to stay current. Verified live across all 2,614 existing rows: $9.58M total labor budget, $24.4M material, $38.2M subcontract, company-wide.
- **Also probed and ruled out, this session:** `GET /jobs/{id}/costs` (200, works, returns a full per-job BELMOS rollup — genuinely unused by any current sync, but `jobCosts/advancedRequest` already gives strictly more detail per-day, so no action taken); `advancedBudgets/material` + `/subcontract` (200, but returned `[]` on the one job tested — inconclusive, would need testing against a job with real subcontracted budget to know if it has content or if it's redundant with the costCodes budget fields above); `payClasses` (403 — blocked by the account's current OAuth scope grant, not fixable in code); everything else in the original endpoint scan (costCodeTransactionInfo, costTypeQuantity, quantityAdjustment, payItemTransaction, a standalone `/equipment`, `/foremen`, `/costTypes`, `rateSets`) — still confirmed 404/dead, no change from prior scans.
- **No front-end or Edge Function code changed.** Both fixes were DB-only (pg_cron job definitions + a generated-column schema change), applied directly via Supabase MCP — not through `deploy.command` or `supabase functions deploy`. Migrations recorded for the paper trail; not yet pushed to `main` (this repo's confirm-before-deploy convention) — see next session or Garrett for whether to push.
